...
interface IPhase1Token {
    function transfer(address to, uint256 amount) external returns (bool);
    function transferFrom(address from, address to, uint256 amount) external returns (bool);
}

contract AgentTabsPhase1 {
    struct Policy {
        address agent;
        address merchant;
    ...
    IPhase1Token public immutable paymentToken;
    ...
    constructor(address token) {
        ...
        paymentToken = IPhase1Token(token);
        admin = msg.sender;
    }
    ...
    function spend(address owner, uint256 amount, bytes32 requestId, bytes32 resultHash) external nonReentrant {
        ...
        require(approvedMerchant[policy.merchant], "MERCHANT_NOT_APPROVED");
